...
export function DraftAdvisor({ steps, meta, mapId, onHeroClick }: DraftAdvisorProps) {
  ...
  const fullComp = useMemo(
    () => (showComp ? suggestFullComp(steps, meta, mapId, compSeed) : null),
    [showComp, steps, meta, mapId, compSeed],
  );
  ...
  return (
    <div className="glass-card rounded-2xl p-4 sm:p-5">
      ...
      <div className="flex items-center justify-between gap-3 mb-4">
        ...
        {mapId && meta.length > 0 && (
          <button
            onClick={() => { if (showComp) setCompSeed(s => s + 1); else setShowComp(true); }}
          ...
          >
            {showComp ? '↻ Another comp' : '★ Suggest Comp'}
          </button>
        )}
      </div>
            ...
            <div className="space-y-2">
              ...
              {analysis.warnings.map((w, i) => (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ duration: 0.25, delay: Math.min(i * 0.05, 0.25), ease }}
                  className="flex items-start gap-2 bg-amber-500/10 border border-amber-500/25 rounded-lg px-2.5 py-2"
                >
            ...
            <div className="space-y-2">
              ...
              {analysis.synergiesFound.map((s, i) => (
                <motion.div
                  ...
                  transition={{ duration: 0.25, delay: Math.min(i * 0.05, 0.25), ease }}
                  className="flex items-center gap-2 bg-positive/5 border-l-4 border-l-positive border border-positive/15 rounded-lg px-2.5 py-1.5"
                >
      ...
      {/* ── Full comp suggestion (full width when toggled) ── */}
      {fullComp && (
        <div className="space-y-2 mt-4 pt-4 border-t border-border">
          <div className="flex items-center gap-3">
            <SectionHeader icon="★" label="Suggested Composition" colorClass="text-accent-light" bgClass="bg-accent/10" borderClass="border-accent/30" />
            <p className="text-xs text-accent italic flex-1 min-w-0 truncate">{fullComp.strategy}</p>
            <button
              onClick={() => setShowComp(false)}
              className="shrink-0 w-6 h-6 rounded-md bg-white/5 hover:bg-white/10 text-text-muted hover:text-text-primary text-sm transition-colors"
              aria-label="Hide composition"
            >
              &times;
            </button>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-1.5">
            {fullComp.heroes.map(({ hero, role, reason }, i) => (
              <motion.button
                ...
                transition={{ duration: 0.28, delay: Math.min(i * 0.05, 0.3), ease }}
                onClick={() => onHeroClick(hero)}
                className="w-full flex items-center gap-2.5 bg-accent/5 border-l-4 border-l-accent border border-accent/15 rounded-lg px-3 py-2.5 hover:border-accent/40 hover:bg-accent/10 transition-colors text-left"
              >
                <span className="text-sm font-bold text-accent/80 w-4 text-center">{i + 1}</span>
                <HeroIcon hero={hero} size="sm" />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-semibold text-text-primary truncate">{hero.name}</span>
                    <span className="text-[10px] font-bold text-accent bg-accent/10 px-1.5 py-0.5 rounded-full uppercase tracking-wider shrink-0">{role}</span>
                  </div>
